...
class CampaignController {
  ...
  private sanitizeFilter = (req: Request): IPaginationFilter => {
    ...
    if (mapSortFilterFields.some((x) => x.incoming == filter.sort.field)) {
      filter.sort.field = mapSortFilterFields
        .filter((map) => map.incoming === filter.sort.field)
        .pop().to;
    }
    return filter;
  };
  ...
  public getAllCampaigns = async (req: Request, res: Response) => {
    ...
    const campaigns: IListResult<Campaign> =
      await this.campaignService.getAllCampaigns(res.locals.user_id, filter);
